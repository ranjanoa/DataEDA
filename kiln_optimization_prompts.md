...
### (Charts first — CRITICAL: paste this before Part 3b)
...
IMPORTANT: Output ALL chart tags FIRST at the top of your response, before any text
analysis. This ensures they are captured before the response length limit is hit.
...
Output these 3 parallel coordinate charts, then interpret each:
...
For Map 1 (color=Production), describe the RED lines (production > 162 t/h):
...
For Map 2 (color=SFC), describe the BLUE lines (SFC < 0.082):
What Total_Fuel_Flow, Pyrometer, and O2 values give the best fuel efficiency?

For Map 3 (color=NOx), describe the RED lines (NOx > 500):
...
Output these 2 3D charts, then interpret each:
...
For 3D Map 1: Where is the Golden Batch cluster? (Z>162, Y>950°C, X in 2.5-3.3%, color blue)
Estimate what % of historical data falls in this optimal 3D region.

For 3D Map 2: Where is the optimal fuel split ridge? (Z>162, color dark blue/purple)
...
Output these charts, then interpret:
